...
from gathercore.interfaces.db.dbcommands import DBCommit, DBInsert, DBSelect
...
def insert_state(db_queue, state_rec: dict):
    sql = (
        f"""insert into track_{state_rec.get("project_id", "")} values  (%s,%s,%s,%s)"""
    )
    params = (
        state_rec.get("id"),
        state_rec.get("time"),
...
def insert_middle(db_queue, rec: dict):
    sql = f"""insert into middle_{rec.get("project_id", "")} values  (%s,%s,%s)"""
    params = (rec.get("m_id"), rec.get("date_time"), rec.get("value"))
    db_queue.put(DBInsert(sql, params))


def insert_idle(db_queue, ilde_rec):
    sql = f"""insert into idles_{
        ilde_rec.get("project_id", "")
    } values  (%s,%s,%s,%s,%s,%s)"""
    params = (
        ilde_rec.get("id"),
        ilde_rec.get("cause"),
...
def querry_causes(db_interface: DBInterface, machine_id: int, project_id: int) -> list:
...
def querry_settings(db_interface: DBInterface, project_id: int):
    sql = f"SELECT * FROM settings_{project_id}"
    reply = db_interface.direct_call(DBSelect(sql))
    return reply


def update_arg_setting(db_queue, arg, val, project_id: int):
    sql = f"UPDATE settings_{project_id} SET value = %s WHERE arg = %s"
    params = (str(val), arg)
    db_queue.put(DBInsert(sql, params))
