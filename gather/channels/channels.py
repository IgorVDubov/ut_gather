...
def parse_attr_params(attrParam: str) -> Tuple[int | None, str | None]:
    '''
    ...
    def __init__(self, id, dbQuie, handler: callable, args: Vars = None) -> None:
        if handler == None:
            raise myexceptions.ConfigException(
                f'No handler at channel {id} params')
            ...
            if self.source.result:
                ...
                if self.source.format == ValTypes.BIT:  # !!!!!   dвынести преобразования в source или mobdus reader
                    ...
                    if self.sourceIndexList is not None or len(self.sourceIndexList) > 0:
                        self.result_in = [self.source.result[i]
                                          for i in self.sourceIndexList]
                    else:
                ...
                self.dost = False
                # print(f'No result in channel {self.id} source {self.source}')                     #TODO обработать отсутствие результата
        else:
            # print(f'No source {self.source} reading channel {self.id} ')                            #TODO обработать отсутствие результата
            self.dost = False
...
class Programm(Channel):
    ...
    def __call__(self):
        try:
            ...
            raise myexceptions.ProgrammException(
                f'Exc in channel {self.id} handler:{self.handler.__name__} raise {e}')
