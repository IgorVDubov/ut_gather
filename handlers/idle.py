...
def idle(vars):
    ...
    if vars.set_cause_flag:
        ...
        logics.current_idle_add_cause(
            vars.machine_id, vars.operator_id, vars.cause_id, datetime.now(), vars.project_id, vars.db_quie)

    # записываем текущий простой и начинаем новый с текущего времени (например в конце смены)
    if vars.restore_idle_flag:
        vars.restore_idle_flag = False
        logics.current_idle_store(
            vars.machine_id, vars.project_id, vars.db_quie)
        logics.current_idle_add_cause(
            vars.machine_id, vars.operator_id, idle.cause_id, datetime.now(), vars.project_id, vars.db_quie)
        ...
        if idle:             # простой уже зафиксирован
            if idle.cause:  # уже есть причина
                if (idle.cause == settings.TECH_IDLE_ID) and ((datetime.now()-idle.cause_time).total_seconds() >= vars.techidle_lenhth):
                    ...
                    print(
                        f'store from backend {(datetime.now()-idle.cause_time).total_seconds() >= vars.techidle_lenhth}')
                    # logics.current_idle_store(vars.machine_id)
                    logics.current_idle_add_cause(
                        vars.machine_id, vars.operator_id, settings.NOT_CHEKED_CAUSE, datetime.now(), vars.project_id, vars.db_quie)
            ...
            logics.current_idle_set(vars.machine_id, vars.state, vars.techidle_lenhth,
                                    vars.operator_id, settings.TECH_IDLE_ID, datetime.now(), datetime.now())
    else:
