...
def r_level_timeout_OROX(vars):
    '''
    обработка 2-х аналоговых сигналов источника (станок ОРОКС)
    по бегущему среднему
    с техпроcтоем
    '''
    # -------------------------
    ...
    # -------------------------
    # 'm_id'                                                          id станка
    # 'result_in':'канал_станка.result_in',                            вход от источника
    # 's2': "канал2_станка.s.result_in",                                        вход от станка ОРОКС
    # 'result': 'канал_станка.result',                                значение канала (бегущее среднее)
    # 'dost':'канал_станка.dost'                                      достоверность канала к источнику
    # 'gr_stand':1                                                    граница откл/простой
    # 'gr_work':8                                                     граница простой/работа
    ...
    #  если нет источника или входящий результат пустой массив
    if vars.result_in is None or vars.s2 is None:
        result_in_error = True
    ...
    if not result_in_error and vars.dost:
        ...
        vars.v2_2 = vars.v2_1
        vars.v2_1 = vars.s2
        result2 = vars.k2 * (vars.v2_1 + vars.v2_2 + vars.v2_3 + vars.v2_4 + vars.v2_5)/5
        if result1 > 0 or result2 > 0:
            dzero = 0.1
        else:
            dzero = 0    
        vars.result = dzero + (result1 * result2)
