...
import settings
from gathercore.channels.channel import Channel
                ...
                if data := json.loads(line):
                    rec = {
                        data.get("id"): Idle(
                            ...
                            data.get("tech_idle"),
                            datetime.strptime(
                                data.get("begin_time"), settings.TIME_FORMAT
                            )
                            ...
                            data.get("cause_id"),
                            datetime.strptime(
                                data.get("cause_time"), settings.TIME_FORMAT
                            )
                            if data.get("cause_time")
                            else None,
                            datetime.strptime(
                                data.get("cause_set_time"), settings.TIME_FORMAT
                            )
                            if data.get("cause_set_time")
                            else None,
                            data.get("length"),
                        )
                    }
        ...
        data = dict(
            {
                ...
                "begin_time": idle.begin_time.strftime(settings.TIME_FORMAT)
                if idle.begin_time
                else None,
                "operator_id": idle.operator,
                "cause_id": idle.cause,
                "cause_time": idle.cause_time.strftime(settings.TIME_FORMAT)
                if idle.cause_time
                else None,
                "cause_set_time": idle.cause_set_time.strftime(settings.TIME_FORMAT)
                if idle.cause_set_time
                else None,
                "length": idle.length,
            }
        )
                ...
                data = dict(
                    {
                        ...
                        "cause_set_time": idle.cause_set_time.strftime(
...
def jsdb_put_state(state_rec: dict):
    raise NotImplementedError("move to dc")
...
def get_current_state(machine_channel: Channel) -> CurrentStateProtocol:
    ...
    if idle := get_current_idle(machine_channel.get_arg("args.m_id")):
        ...
        saved_state_time = (
            idle.begin_time.strftime(settings.TIME_FORMAT) if idle.begin_time else None
        )
        saved_current_cause = idle.cause
        saved_current_cause_time = (
            idle.cause_time.strftime(settings.TIME_FORMAT) if idle.cause_time else None
        )
        saved_current_cause_set_time = (
            idle.cause_set_time.strftime(settings.TIME_FORMAT)
            if idle.cause_set_time
            else None
        )
    else:
        ...
        if current_idle.cause != cause_id:
            ...
            if current_idle.cause is not None:
                ...
                if current_idle.cause == settings.TECH_IDLE_ID:
                    ...
                    logger.log(
                        "PROG", f"{machine_id} add cause {cause_id} to tech timeout"
                    )
                    ...
                    logger.log(
                        "PROG", f"{machine_id} add cause {cause_id} to NOT_CHEKED_CAUSE"
                    )
                    # current_idle.cause_time  не меняется
                else:
...
def save_current_idle_v2(
    machine_id: int, prj_id: int, buffer_time: int, db_queue: DBInterface
):
    """
    сохраняем простой в БД
