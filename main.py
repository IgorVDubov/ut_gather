...
def init():
    ...
    db_processor = DBCommandProcessor(
        create_db_connector(config.DB_TYPE, config.DB_PARAMS))
    ...
    print(channel_base)
    print(
        f'Modbus Exchange Server: {config.modbus_server_params["host"]}, {config.modbus_server_params["port"]}')
