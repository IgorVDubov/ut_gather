...
class MainHtmlHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.render('index.html', 
                    ...
                    idle_couses=json.dumps(logics.convert_none_2_str(logics.get_machine_causes)(machine_id), default=str),
                    current_state=logics.convert_none_2_str(logics.get_current_state)(self.application.data.channelBase,machine_id),
...
handlers=[
        ...
        (r"/static/(.*)", StaticFileHandler, {"path": os.path.join(*path_params.get ('static','web/webdata').split('/'))}),
        (r'/js/(.*)', StaticFileHandler, {"path": os.path.join(*path_params.get ('js','web/webdata/js').split('/'))}),
        (r'/css/(.*)', StaticFileHandler, {"path": os.path.join(*path_params.get ('css','web/webdata/css').split('/'))}),
        (r'/images/(.*)', StaticFileHandler, {"path": os.path.join(*path_params.get ('images','web/webdata/images').split('/'))}),
        ]
